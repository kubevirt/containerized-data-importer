...
> Note: CDI is also able to import a local file endpoint but this is not supported except for testing and debugging. The `file::///path-to-host-file` scheme is used and the endpoint's host path must be mounted in the importer container. This bind-mount is not done via the CDI controller nor importer pod.
