...
unit-test: $(IMPORTER_BUILD)/Dockerfile
	@echo '********'
	@echo 'Building unit test image'
	cd $(REPO_ROOT)/build/importer && \
		docker build -t cdi-unit-tests
	docker run --rm --entrypoint ['/bin/bash'] -v $(REPO_ROOT):/go/src/kubevirt.io/containerized-data-importer:Z -w /go/src/kubevirt.io/containerized-data-importer cdi-unit-tests
