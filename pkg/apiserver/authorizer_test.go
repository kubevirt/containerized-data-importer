...
func fakeRequest() *restful.Request {
	...
	req.Request.URL.Path = "/apis/upload.cdi.kubevirt.io/v1alpha1/namespaces/default/uploadtoken"
	return req
}
...
func TestGenerateAccessReviewPathErrGroup(t *testing.T) {
	app := newAuthorizor()
	req := fakeRequest()
	req.Request.URL.Path = "/apis/NOTOURGROUP/v1alpha1/namespaces/default/uploadtoken"
	authReview, err := app.generateAccessReview(req)
