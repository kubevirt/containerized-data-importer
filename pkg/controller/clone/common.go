...
func SameVolumeMode(sourceVolumeMode *corev1.PersistentVolumeMode, others ...*corev1.PersistentVolumeClaim) bool {
	if sourceVolumeMode == nil {
		return false
	}
	for _, pvc := range others {
		if *pvc.Spec.VolumeMode != *sourceVolumeMode {
			return false
		}
	}
	return true
