...
func createTempSourceClaim(ctx context.Context, namespace string, targetClaim *corev1.PersistentVolumeClaim, snapshot *snapshotv1.VolumeSnapshot, client client.Client) (*corev1.PersistentVolumeClaim, error) {
	...
	volumeMode, err := getVolumeModeForTempSourceClaim(snapshot, vsc, fallbackVolumeMode)
	if err != nil {
		return nil, err
	}
...
func getVolumeModeForTempSourceClaim(snapshot *snapshotv1.VolumeSnapshot, vsc *snapshotv1.VolumeSnapshotContent, fallback *corev1.PersistentVolumeMode) (*corev1.PersistentVolumeMode, error) {
