...
func (ic *ImportController) syncPvc(key string) error {
	...
	glog.V(Vdebug).Infof("syncPvc: next pvc to process: %s\n", key)
...
func (ic *ImportController) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	ic.Controller.Run(threadiness, stopCh)
	for i := 0; i < threadiness; i++ {
		go wait.Until(ic.runPVCWorkers, time.Second, stopCh)
	}
	<-stopCh
	return nil
}
