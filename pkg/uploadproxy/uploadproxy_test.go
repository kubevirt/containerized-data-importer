...
var _ = Describe("submit request and check status", func() {
	table.DescribeTable("Test proxy status code", func(path string, statusCode int) {
		...
		table.Entry("Test Archive OK", common.UploadArchivePath, http.StatusOK),
		table.Entry("Test Archive error", common.UploadArchivePath, http.StatusInternalServerError),
