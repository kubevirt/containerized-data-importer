...
var _ = Describe("Import Proxy tests", func() {
	...
	Describe("importing from cdi proxied web server", func() {
		...
		FDescribeTable("should", func(args importProxyTestArguments) {
			...
			Eventually(func() bool {
				if importerPod, err := f.K8sClient.CoreV1().Pods(f.Namespace.Name).Get(context.TODO(), podName, metav1.GetOptions{}); err == nil {
					...
					//Only print the following message when the test is bypassing the proxy
					if args.expected() == BeFalse() {
						fmt.Fprintf(GinkgoWriter, "INFO: Checking importer POD %s: requests were not proxied\n", podName)
						return false
					}
				} else {
				...
				return false
			}, time.Second*120, time.Second).Should(args.expected())
