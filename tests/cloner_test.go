...
var _ = Describe("all clone tests", func() {
	var _ = Describe("[rfe_id:1277][crit:high][vendor:cnv-qe@redhat.com][level:component]Cloner Test Suite", Serial, func() {
		...
		commonCloneTests := func(storageClass, cloneType string) {
			...
			var tableEntries = []TableEntry{
				Entry("[test_id:8492]Block to block (empty storage size)", v1.PersistentVolumeBlock, v1.PersistentVolumeBlock, false, decorators.RequiresBlockStorage),
				Entry("[test_id:8491]Block to filesystem (empty storage size)", v1.PersistentVolumeBlock, v1.PersistentVolumeFilesystem, false, decorators.RequiresBlockStorage),
			}
			if cloneType != "copy" {
		...
		smartCloneTests := func(cloneType string) {
			Context("WaitForFirstConsumer with advanced cloning methods", func() {
				...
				AfterEach(func() {
					By("Restoring original WFFC clone strategy")
					if wffcOriginalProfileSpec != nil && wffcStorageClass != nil {
						Expect(utils.UpdateStorageProfile(f.CrClient, wffcStorageClass.Name, *wffcOriginalProfileSpec)).To(Succeed())
					}
				})
		...
		blockToFilesystemTest := func(cloneType string) {
			var tableEntries = []TableEntry{
		...
		Context("HostAssisted Clone", func() {
			...
			commonCloneTests(cloneStorageClassName, "copy")
			copyCloneTests()
			blockToFilesystemTest("copy")
		})
		...
		Context("SmartClone", decorators.RequiresSnapshotStorageClass, func() {
			...
			commonCloneTests(cloneStorageClassName, "snapshot")
			smartCloneTests("snapshot")
			blockToFilesystemTest("snapshot")
		})
		...
		Context("[rfe_id:4219]CSI Clone", decorators.RequiresCSICloneClass, func() {
			...
			commonCloneTests(cloneStorageClassName, "csi-clone")
			smartCloneTests("csi-clone")
		})
	...
	var _ = Describe("Namespace with quota", Serial, func() {
		...
		var (
			...
			orgProfileSpec *cdiv1.StorageProfileSpec
		...
		It("[test_id:9076]Should fail clone data across namespaces, if target namespace doesn't have enough quota", decorators.RequiresNoCsiDriver, func() {
			err := f.UpdateCdiConfigResourceLimits(int64(0), int64(512*1024*1024), int64(1), int64(512*1024*1024))
			Expect(err).NotTo(HaveOccurred())
			pvcDef := utils.NewPVCDefinition(sourcePVCName, "500M", nil, nil)
			pvcDef.Namespace = f.Namespace.Name
	...
	var _ = Describe("[rfe_id:1277][crit:high][vendor:cnv-qe@redhat.com][level:component]Cloner Test Suite", func() {
		...
		var _ = Describe("Host-assisted clone", func() {
			...
			It("[test_id:4000] Create a data volume and then clone it while killing the container and verify retry count", decorators.RequiresNoCsiDriver, func() {
