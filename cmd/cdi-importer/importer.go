...
func newDataSource(source string, contentType string, volumeMode v1.PersistentVolumeMode) importer.DataSourceInterface {
	...
	pullMethod, _ := util.ParseEnvVar(common.ImporterPullMethod, false)
	imageRootDir, _ := util.ParseEnvVar(common.ImporterImageRootDir, false)
	...
	case cc.SourceRegistry:
		ds := importer.NewRegistryDataSource(ep, acc, sec, registryImageArchitecture, certDir, insecureTLS, pullMethod, imageRootDir, envFile)
		return ds
	case cc.SourceS3:
