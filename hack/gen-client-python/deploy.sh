...
# Remove content under kubevirt_cdi, docs and test directories
rm -rf "${CLIENT_PYTHON_DIR}"/{kubevirt_cdi,docs,test}
...
for i in api_client.py configuration.py; do
    if [ "$(git diff --numstat -- "kubevirt_cdi/${i}" | cut -f 1)" != "1" ] &&
        [ -n "$(git diff --numstat -- "kubevirt_cdi/${i}" | cut -f 1)" ]; then
        CLIENT_UPDATED="true"
    fi
done
...
if git status --porcelain |
    grep 'kubevirt_cdi/' |
    grep -v 'kubevirt_cdi/\(api_client[.]py\|configuration[.]py\)' |
    grep --quiet "^ [AM]"; then
